...
| core-02 (完了済み) | 中 | `packages/core/src/core/ChatProcessor.ts` | `runToolLoop` の Claude/非Claude 分岐とメッセージ組み立てを小さな private 関数に分割し、`any` キャストを削減。 | tool 連鎖の挙動（tool_use → tool_result → 継続）が既存と同じか。 |
| core-03 (完了済み) | 低 | `packages/core/src/core/AITuberOnAirCore.ts` | Provider の switch を `buildChatServiceOptions(providerName, baseOptions, providerOptions)` のような関数へ抽出して可読性を改善。 | 既存の provider 選択と options のマージ順序が変わらないか。 |
| core-04 (完了済み) | 中 | `packages/core/src/core/AITuberOnAirCore.ts` | `processChat` / `processVisionChat` の重複した排他制御・イベント発火・try/finally を共通化（小さな helper へ）。 | PROCESSING_START/END が同じタイミングで発火するか。 |
...
| core-06 (完了済み) | 中 | `packages/core/src/services/chat/providers/*/*Summarizer.ts` | 3実装の共通処理（prompt 生成、会話結合、fallback）を共有 helper へ抽出。各 provider は request 生成だけを担当。 | 例外時の fallback 文言が維持されるか。 |
