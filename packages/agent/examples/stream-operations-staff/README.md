...
7. `Mikoの音声` は初期状態が `OFF`、AivisSpeechは`未確認`です。`ブラウザ標準（Web Speech）` または`AivisSpeech`を選ぶと、その後に新しく作成された観測カードだけを「観測→提案」の順で読み上げます。AivisSpeechは選択時にローカル接続を確認します。
...
3. `Mikoの音声` から `AivisSpeech（ローカル）` を選びます。
4. 接続確認後に `AivisSpeech: 接続済み` となり、話者を選択できます。

画面はAivisSpeechの選択時にブラウザから `/version` へ短時間の疎通確認を行い、続けて `@aituber-onair/voice` の話者一覧APIで `/speakers` を取得します。これにより、ViteのオリジンからのCORS許可も検証されます。未起動・到達不能・CORS不許可の場合も選択値はAivisSpeechのまま保持し、`接続できません` と接続エラーを表示します。起動・再接続後は `再確認` を押してください。
