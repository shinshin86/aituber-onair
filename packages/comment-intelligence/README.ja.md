...
- プロンプトインジェクション、URL、スパム、繰り返し、非建設的な否定コメントなどをルールベースで検出します。
- 正規化済みコメントをランキングします。
- LLMなしで未選択コメントを要約します。
...
視聴者ごとの安全状態や嫌なコメント検知は、返答対象を選ぶためのガードです。危険なコメントや荒れやすいコメントをAITuberに読ませないために使い、配信全体のモデレーションはプラットフォームのBANや人間のモデレーターと併用してください。
