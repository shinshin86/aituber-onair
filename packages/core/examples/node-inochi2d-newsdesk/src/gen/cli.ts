...
import {
  createInochi2DAvatar,
  type Inochi2DAvatarDiagnostics,
} from './inochi2dAvatar.js';
...
interface RenderSummary {
  ...
  avatarDiagnostics: Inochi2DAvatarDiagnostics;
  motionSamples: Array<
    MotionDiagnostics & {
      frame: number;
      time: number;
...
function createPaths(
  ...
  return {
    ...
    configPath: path.join(
      parsed.dir,
      `${parsed.name}.inochi2d-gen.config.json`,
    ),
    tempDir: path.join(
      parsed.dir,
      `.inochi2d-gen-${parsed.name}-${process.pid}`,
    ),
  };
}
...
async function render(
  ...
  const avatar = await createInochi2DAvatar(config);
      ...
      writeFrames: async (stdin: Writable) => {
        for (let frame = 0; frame < totalFrames; frame += 1) {
          ...
          if ((frame + 1) % config.fps === 0 || frame === totalFrames - 1) {
            console.error(
              `Rendered ${frame + 1}/${totalFrames} Inochi2D frames ` +
...
function parseScript(raw: string): NewsdeskScript {
  ...
  if (typeof value.avatar !== 'string' || value.avatar.trim() === '') {
    throw new Error('script.json must include an Inochi2D .inx avatar path.');
  }
  if (!value.avatar.toLowerCase().endsWith('.inx')) {
    throw new Error('script.json avatar must point to an .inx file.');
  }
  if (
    typeof value.inochi2dRuntime !== 'string' ||
    value.inochi2dRuntime.trim() === ''
  ) {
    throw new Error(
      'script.json must include the local Inochi2D runtime directory.',
    );
  }
  if (
    value.avatarMotion !== undefined &&
    (typeof value.avatarMotion !== 'string' ||
      !value.avatarMotion.toLowerCase().endsWith('.json'))
  ) {
    throw new Error('script.json avatarMotion must point to a JSON file.');
  }
...
export async function main(argv = process.argv.slice(2)): Promise<void> {
  ...
  try {
    ...
    const config: RenderConfig = {
      ...
      avatarMotion: script.avatarMotion
        ? resolveFrom(scriptPath, script.avatarMotion)
        : null,
      inochi2dRuntime: resolveFrom(scriptPath, script.inochi2dRuntime),
      motion: {
