...
export function validateScript(script: unknown): string[] {
  ...
  if (
    ...
    script.avatar.trim() !== '' &&
    !script.avatar.toLowerCase().endsWith('.inx')
  ) {
    errors.push('script.avatar must point to an .inx file.');
  }
  if (script.avatarMotion !== undefined) {
    requireString(script.avatarMotion, 'script.avatarMotion', errors);
  }
  if (
    typeof script.avatarMotion === 'string' &&
    script.avatarMotion.trim() !== '' &&
    !script.avatarMotion.toLowerCase().endsWith('.json')
  ) {
    errors.push('script.avatarMotion must point to a JSON file.');
  }
  requireString(script.inochi2dRuntime, 'script.inochi2dRuntime', errors);
