...
const validScript: NewsdeskScript = {
  ...
  avatarFraming: { scale: 0.65, x: 0, y: 1450 },
  motion: { intensity: 1 },
  blinkSeed: 42,
  lines: [
...
describe('strict schema and provenance', () => {
  ...
  it('validates optional avatar framing overrides', () => {
    ...
    expect(
      ...
      /scale must be at least 0.01|x must be at most 10000|extra is not allowed/,
    );
  });

  it('requires valid model, motion, and runtime paths', () => {
    expect(validateScript({ ...validScript, avatarMotion: undefined })).toEqual(
      [],
    );
    expect(
      validateScript({
        ...validScript,
        avatar: '/path/to/avatar.json',
        avatarMotion: '/path/to/avatar.motion.bin',
        inochi2dRuntime: '',
      }).join('\n'),
    ).toMatch(
      /avatar must point to an .inx|avatarMotion must point to a JSON|inochi2dRuntime must be a non-empty string/,
    );
  });
...
describe('Core chat providers', () => {
  ...
  it('normalizes generated scripts to license-safe local placeholders', () => {
    const outputPath = path.join(projectRoot, 'samples', 'news.json');
    const normalized = normalizeScriptForOutput(validScript, outputPath);
    expect(normalized.avatar).toBe('/path/to/inochi2d/models/avatar.inx');
    expect(normalized.avatarMotion).toBe(
      '/path/to/inochi2d/models/avatar.motion.json',
    );
    expect(normalized.inochi2dRuntime).toBe('/path/to/inochi2d/runtime');
