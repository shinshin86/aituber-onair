...
function validateAvatarFraming(framing: unknown, errors: string[]): void {
  ...
  validateOptionalNumber(framing.scale, 'script.avatarFraming.scale', errors, {
    min: 0.01,
    max: 10,
  });
  validateOptionalNumber(framing.x, 'script.avatarFraming.x', errors, {
    min: -2,
    max: 3,
  });
  validateOptionalNumber(framing.y, 'script.avatarFraming.y', errors, {
    min: -2,
    max: 3,
  });
}

function validateAvatarMotion(motion: unknown, errors: string[]): void {
  if (!isObject(motion)) {
    errors.push('script.avatarMotion must be an object.');
    return;
  }
  addUnknownFieldErrors(
    motion,
  ...
  if (motion.idle !== undefined)
    requireString(motion.idle, 'script.avatarMotion.idle', errors);
}
...
export function validateScript(script: unknown): string[] {
  ...
  if (
    typeof script.avatar === 'string' &&
    script.avatar.trim() !== '' &&
    !script.avatar.toLowerCase().endsWith('.model3.json')
  ) {
    errors.push('script.avatar must point to a .model3.json file.');
  }
  requireString(script.cubismCore, 'script.cubismCore', errors);
  if (
    typeof script.cubismCore === 'string' &&
    script.cubismCore.trim() !== '' &&
    !script.cubismCore.toLowerCase().endsWith('live2dcubismcore.min.js')
  ) {
    errors.push('script.cubismCore must point to live2dcubismcore.min.js.');
  }
