...
describe('strict schema and provenance', () => {
  ...
  it('validates optional avatar framing overrides', () => {
    ...
    expect(
      ...
      /scale must be at least 0.01|x must be at most 3|extra is not allowed/,
    );
  });

  it('requires valid Cubism Core/model paths and validates idle motion', () => {
...
describe('Core chat providers', () => {
  ...
  it('normalizes generated scripts to license-safe local placeholders', () => {
    ...
    expect(normalized.avatar).toBe(
      '/path/to/live2d_models/model/runtime/model.model3.json',
    );
    expect(normalized.cubismCore).toBe(
      '/path/to/CubismSdkForWeb/Core/live2dcubismcore.min.js',
    );
