...
function createPaths(
  ...
  return {
    ...
    configPath: path.join(parsed.dir, `${parsed.name}.psd-gen.config.json`),
    tempDir: path.join(parsed.dir, `.psd-gen-${parsed.name}-${process.pid}`),
  };
}
...
async function render(
  ...
  const stateFrames: Record<AvatarStateKey, number> = {
