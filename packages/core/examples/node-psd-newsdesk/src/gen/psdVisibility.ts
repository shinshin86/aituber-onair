import type { PsdModel, PsdModelNode } from './psdModel.js';
...
/** Create initial visibility and keep only the first visible radio sibling. */
export function getInitialVisibility(model: PsdModel): PsdVisibilityOverrides {
  const next: PsdVisibilityOverrides = {};
  for (const node of Object.values(model.nodes)) {
    if (!node.forcedVisible) next[node.id] = isNodeVisibleByDefault(node);
  }

  const normalizeRadioSiblings = (siblingIds: string[]): void => {
    const radioChildIds = siblingIds.filter(
      (childId) => model.nodes[childId]?.radio,
    );
  ...
  for (const node of Object.values(model.nodes)) {
    if (node.kind === 'group') normalizeRadioSiblings(node.childIds);
  }
  return next;
}
...
/** Resolve visible pixel layers after applying mouth and eye roles. */
