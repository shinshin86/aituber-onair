...
function validateAvatarRoles(roles: unknown, errors: string[]): void {
  if (!isObject(roles)) {
    errors.push('script.avatarRoles must be an object.');
    return;
  }
  const roleNames = ['mouthOpen', 'mouthClosed', 'eyesOpen', 'eyesClosed'];
  addUnknownFieldErrors(
    roles,
    new Set(roleNames),
    'script.avatarRoles',
  ...
  for (const role of roleNames) {
    if (roles[role] !== undefined) {
      requireString(roles[role], `script.avatarRoles.${role}`, errors);
    }
  }
}
