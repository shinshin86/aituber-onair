...
const validScript: NewsdeskScript = {
  ...
  avatarLayout: { scale: 1, x: 0.5, y: 0.45 },
  motion: { intensity: 1 },
  blinkSeed: 42,
  lines: [
...
describe('strict schema and provenance', () => {
  ...
  it('validates optional PSD role path overrides strictly', () => {
    expect(
      validateScript({
        ...validScript,
        avatarRoles: {
...
describe('Core chat providers', () => {
  ...
  it('normalizes output paths beside the requested JSON file', () => {
    ...
    expect(normalizeScriptForOutput(validScript, outputPath).avatar).toBe(
      '../assets/sample-static.psd',
    );
