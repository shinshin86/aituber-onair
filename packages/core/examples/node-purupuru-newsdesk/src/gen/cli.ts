...
import { loadPuruPuruPackage, type MouthState } from './purupuruPackage.js';
import {
  createRenderer,
...
function createPaths(
  ...
  return {
    ...
    configPath: path.join(
      parsed.dir,
      `${parsed.name}.purupuru-gen.config.json`,
    ),
    tempDir: path.join(
      parsed.dir,
      `.purupuru-gen-${parsed.name}-${process.pid}`,
    ),
  };
}
...
async function render(
  ...
  const [audio, avatar] = await Promise.all([
    ...
    loadPuruPuruPackage(config.avatar),
  ]);
