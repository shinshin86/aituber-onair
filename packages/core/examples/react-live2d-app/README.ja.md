...
- 設定ダイアログの「アバター・モーション」で、モデルごとに発話感情と
  モーションを割り当て。選んだモーションは「再生」ボタンで確認できます
...
「アバター・モーション」では、モデルの `.model3.json` に登録された
モーションを `happy`、`sad` などの感情に割り当てられます。設定はモデルごとに
ブラウザへ保存されます。割り当てのない感情ではモーションを再生しません。
割り当てたモーションは発話中に指定区間を繰り返し、リップシンクと併用されます。
モーションの前後で通常表情に戻る場合は、開始・終了位置を調整してください。
発話後のアイドルモーションはモデルの標準設定に従います。
視覚エフェクトの設定はモーションの再生に影響しません。
