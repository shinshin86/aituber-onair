...
- Chat providers include `deepseek` and `mistral` in addition to the existing OpenAI, Gemini, Claude, Z.ai, Kimi, xAI, OpenRouter, Gemini Nano, and OpenAI-compatible options
...
- Use TTS engines: `openai`, `geminiTts`, `openaiCompatible`, `voicevox`,
  ...
  `unrealSpeech`, `elevenLabs`, `inworld`, `piperPlus`, `none`
