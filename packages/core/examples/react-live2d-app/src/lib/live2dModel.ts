...
export function installLive2DBlobUrlFix(): void {
	...
	XMLHttpRequest.prototype.open = function (
		...
		const urlString =
			typeof url === "string" ? normalizeBlobUrl(url) : normalizeBlobUrl(url.toString());
...
async function rewriteModelJsonReferences(
	...
	rewrite: (targetPath: string | undefined, mode: "blob" | "data") => Promise<string | undefined>,
) {
...
function createModelJsonBlobUrl(modelJson: Live2DModelJson, revokers: string[]) {
